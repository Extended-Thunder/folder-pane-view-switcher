...
var Utilities = class extends ExtensionCommon.ExtensionAPI {
    getAPI(context) {
        ...
        async function onDragOver(event) {
            ...
            currentFolder = mail3Pane.gFolderTreeView.getFolderAtCoords(
                event.clientX,
                event.clientY
            );
            FPVS.log(
                "dragover, currentFolder, ownerglobal",
                currentFolder,
        ...
        var folderListener = {
            msgsMoveCopyCompleted: function (
                ...
                if (aDestFolder == currentFolder) {
                    ...
                    FPVS.notifyTools.notifyBackground({
                        command: "folderListener",
                        type: "msgsMoveCopyCompleted"
                    });
                } else {
        ...
        return {
            Utilities: {
                ...
                initFolderPaneOptionsPopup: async function () {
                    let mail3Pane = Components.classes[
                ...
                registerListener: async function (eventType, DOMid, windowId) {
                    ...
                    item.addEventListener(
                        "mouseout",
                        FPVSlisteners.onDragExit,
                        false
                    ); //dragleave
                    item.addEventListener(
                        "dragend",
                        FPVSlisteners.onDragDrop,
                        false
                    );
                    item.addEventListener(
                        "dragenter",
                        FPVSlisteners.onDragEnter,
                        false
                    );
                    ...
                    ns.addListener(
                        folderListener,
                        ns.msgsMoveCopyCompleted | ns.folderMoveCopyCompleted
                    );
                },

                unregisterListener: async function (
                    eventType,
                    DOMid,
                    windowId
                ) {
                ...
                showViewInMenus: async function (view, enabled) {
                    ...
                    let mail3PaneWindow = Components.classes[
                        "@mozilla.org/appshell/window-mediator;1"
                    ]
                        .getService(Components.interfaces.nsIWindowMediator)
                        .getMostRecentWindow("mail:3pane");
                    ...
                    let item = mail3PaneWindow.document.querySelector(
                        `#folderPaneOptionsPopup [value=${view}]`
                    );
                    if (item != null) {
                        item.hidden = !enabled;
                    }
                    item = mail3PaneWindow.document.querySelector(
...
Services.scriptloader.loadSubScript(
    FPVS.extension.rootURI.resolve("chrome/content/scripts/notifyTools.js"),
    FPVS,
    "UTF-8"
);
